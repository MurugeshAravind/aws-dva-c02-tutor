...
export const D2_CONCEPTS: Concept[] = [
  {
    ...
    questions: [
      ...
      { t: "s", q: "A developer accidently committed AWS credentials to a public GitHub repo. First action?", o: ["Deactivate the exposed credentials immediately in IAM", "Rotate them within 24 hours", "Delete the GitHub commit", "Enable MFA"], c: [0], e: ["Deactivate immediately to stop unauthorized use — assume they're already compromised.", "Rotating without deactivating leaves the original keys active.", "Deleting the commit doesn't revoke already-seen credentials.", "MFA is a good practice but doesn't revoke the exposed keys."] },
      { t: "s", q: "Which IAM policy element explicitly blocks an action regardless of any Allow?", o: ["Effect: Deny", "Effect: Allow with Condition", "NotAction", "Principal: *"], c: [0], e: ["An explicit Deny statement always overrides any Allow.", "A conditional Allow doesn't override an explicit Deny.", "NotAction restricts which actions an Allow applies to.", "Principal specifies who the policy applies to."] },
      { t: "m", q: "Which are valid IAM policy types? (choose two)", o: ["Identity-based policies attached to a user or role", "Resource-based policies attached to a resource (e.g., S3 bucket policy)", "Network policies controlling VPC routing", "Billing policies setting cost limits", "Database policies controlling RDS queries"], c: [0, 1], e: ["Identity-based policies are the most common IAM policy type.", "Resource-based policies (bucket policies, Lambda resource policies) are standard.", "VPC routing is controlled by route tables, not IAM.", "Budget alerts exist but aren't IAM policy types.", "RDS uses database-level permissions, not IAM policy types."] },
    ],
  },
  {
    ...
    questions: [
      ...
      { t: "s", q: "A web app uses Cognito User Pools for login. Which token should it use to call its own API?", o: ["The ID token (JWT) to pass user identity to the backend", "The refresh token for every API call", "The AWS access key from the Identity Pool", "A hardcoded API key"], c: [0], e: ["The ID token carries user claims and is validated by the API backend.", "Refresh tokens are for getting new access/ID tokens, not for API calls.", "AWS credentials are for calling AWS services directly, not your own API.", "Hardcoded keys are a security risk."] },
      { t: "s", q: "What is the recommended architecture when users need to call DynamoDB directly from a mobile app via Cognito?", o: ["User Pool authentication → Identity Pool for temporary AWS credentials → DynamoDB", "User Pool authentication → DynamoDB directly with JWT", "Identity Pool alone with no User Pool", "API Gateway + Lambda + DynamoDB (no Cognito needed)"], c: [0], e: ["The standard pattern: authenticate with User Pool, exchange for AWS creds via Identity Pool.", "JWTs don't authorize direct DynamoDB calls.", "An Identity Pool alone has no authentication directory.", "That's a valid pattern but doesn't use Cognito for direct SDK access."] },
    ],
  },
  {
    ...
    questions: [
      ...
      { t: "s", q: "A developer must encrypt an S3 object using a key they fully control. Which option gives the most control?", o: ["SSE-C (customer-provided key)", "SSE-S3 (S3-managed key)", "SSE-KMS with AWS-managed key", "No encryption"], c: [0], e: ["SSE-C lets you provide your own key material — you're fully responsible for key management.", "SSE-S3 uses AWS-managed keys with no customer control.", "SSE-KMS with an AWS-managed key is controlled by AWS, not the customer.", "Not encrypting is insecure."] },
      { t: "s", q: "Which action in KMS generates a data key you can use locally to encrypt large amounts of data?", o: ["GenerateDataKey", "Encrypt", "Decrypt", "CreateKey"], c: [0], e: ["GenerateDataKey returns a plaintext key for local encryption and an encrypted copy for storage.", "Encrypt is for small data directly via KMS API.", "Decrypt decrypts already-encrypted data.", "CreateKey creates a CMK, not a data key."] },
    ],
  },
  {
